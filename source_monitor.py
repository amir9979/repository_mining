...
def SourceMonitor(sourceMonitorEXE, sources_dir, out_dir):
    xml="""
...
</sourcemonitor_commands>""".replace("verP", out_dir).replace("verREPO", sources_dir)
    xmlPath = os.path.join(out_dir, "sourceMonitor.xml")
    with open(xmlPath, "wb") as f:
        f.write(xml)
    return Popen([sourceMonitorEXE, "/C", xmlPath]).communicate()

SourceMonitor(*sys.argv[1:])
