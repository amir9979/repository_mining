...
class DataExtractor(object):
        ...
        commited_files = list(map(lambda c:
                                  list(map(lambda file_name:
                                           [file_name, c._commit_id,
                                            c._bug_id, c._commit_date, self.get_commit_url(c._commit_id), c.get_issue_url()],
    ...
    def _store_commits(self):
        ...
        commits = list(map(lambda c: [c._commit_id, c._bug_id, c._commit_date, self.get_commit_url(c._commit_id), c.get_issue_url()], self.commits))
    ...
    def _commits_and_issues(repo, issues):
        ...
        for git_commit in repo.iter_commits():
            ...
            commits.append(
                Commit.init_commit_by_git_commit(git_commit, bug_id, issues.get(bug_id)))
        return commits
