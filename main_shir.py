...
def write_commit(list_of_commit):
    if not os.path.exists(str(pathlib.Path().absolute()) + '/apache_repos/' + NAME_PROJECT):
        os.mkdir(str(pathlib.Path().absolute()) + '/apache_repos/' + NAME_PROJECT)
...
def write(contents, commit, file, name="after"):
    if name == "before":
        if not os.path.exists(str(pathlib.Path().absolute()) + '/apache_repos/' + NAME_PROJECT + '/' + str(commit)):
            os.mkdir(str(pathlib.Path().absolute()) + '/apache_repos/' + NAME_PROJECT + '/' + str(commit))
        os.mkdir(str(pathlib.Path().absolute()) + '/apache_repos/' + NAME_PROJECT + '/' + str(commit) + "/" + file)

    with open(str(pathlib.Path().absolute()) + '/apache_repos/' + NAME_PROJECT + '/' + str(
            commit) + "/" + file + "/" + name +
              ".java", 'w') as file:
...
if __name__ == '__main__':
    ...
    obj_git = GetCommit('D:/a/runner_measure/runner_measure/my-tools/apache/camel', commits_start, commits_end)
